...
[^utf8encoding]: I actually don't know whether it's true that this encoding can fail. Swift strings are always valid Unicode [@noauthor_string_nodate], and UTF-8 can encode all valid Unicode. I suspect that the function is failable because you can try to encode strings using encodings such as ASCII, which can fail for strings that contain characters not encodable as ASCII.
...
- willingness to have failing tests that i later came back and fixed
