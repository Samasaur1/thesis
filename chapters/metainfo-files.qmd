...
When a v1 torrent only contains a single file, its metainfo file uses the `length` key instead of the `files` key and also uses the `name` key from outside the info dictionary. The file will be saved with the name equal to the value of the `name` key, and it is `length` bytes long.
