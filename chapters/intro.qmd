...
![We talk to peers to get the actual file](figures/intro-bittorrent-process-overview-peers.tex){#fig-intro-bittorrent-process-overview-peers}
...
- **Written in memory-unsafe languages like C/C++**. C and C++ are notorious for their propensity to harbor subtle memory safety bugs, which many modern languages like Swift prevent automatically.
...
This document aims to explain BitTorrent while assuming as little background as possible. To that end, I have provided various "primers" on some core concepts that I refer to frequently as appendices. These obviously are not comprehensive, and better materials can no doubt be found on the internet, but these should provide enough knowledge to make the rest of the thesis understandable.

In pursuit of the same goal, the chapters follow a similar structure. Each chapter begins with a conceptual overview of the topic that should be approachable to everyone before delving into increasing levels of detail, culminating in implementation-level details that may not interest all audiences.
